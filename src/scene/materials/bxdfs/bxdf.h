...
{
public:
    virtual float Evaluate(const float &costheta) = 0;
};

class FresnelNo : public Fresnel
{
public:
    virtual float Evaluate(const float &costheta) {return 1.0f;}
};

class FresnelDielectric: public Fresnel
{
public:
    FresnelDielectric(): FresnelDielectric(1.0f,1.0f) {}
    FresnelDielectric(const float ei,const float et): etai(ei),etat(et) {}

    virtual float Evaluate(const float &costheta);
    float FreDiel(const float& ei, const float& cosi, const float& et, const float& cost);
    float etai,etat;
};
...
//Constructors/Destructors
    BxDF(BxDFType t) : type(t), name("BxDF"){fresnel = NULL;}
    virtual ~BxDF(){}
//Functions
    ...
    virtual glm::vec3 EvaluateScatteredEnergy(const glm::vec3 &wo, const glm::vec3 &wi, float &pdf) const = 0;
    ...
    inline bool MatchesFlags(const BxDFType& flags) {return (flags & type) == type;}

    bool isPerfectReflective(const glm::vec3& wo,const glm::vec3& wi) const ;

    glm::vec3 SphericalDirection(float sintheta,float costheta,float phi) const;
