...
Integrator* XMLReader::LoadIntegrator(QXmlStreamReader &xml_reader)
{
    ...
    else if(QStringRef::compare(type, QString("SHLightingIntegrator")) == 0)
    {
        result = new SHLightingIntegrator();
    }
    else if(QStringRef::compare(type, QString("photonIntegrator")) == 0)
    {
        result = new PhotonIntegrator();
    }
    else
    {
        result = new DirectLightingIntegrator();
