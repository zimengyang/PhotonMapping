...
Intersection Sphere::RandomSampleOnSurface(const float &u, const float &v)
{
    ...
    localSamplePoint = localSamplePoint ;
    glm::vec3 worldSamplePoint(glm::vec3(transform.T() * glm::vec4(localSamplePoint, 1.0f)));

    Intersection isx;
    isx.t = 1.0f;
    isx.point = worldSamplePoint;
    isx.texture_color = Material::GetImageColor(GetUVCoordinates(localSamplePoint), material->texture);
    isx.object_hit = this;
    SetNormalTangentBitangent(localSamplePoint,isx);
    isx.point = isx.point + 1e-3f*isx.normal;
...
glm::vec3 Sphere::Sample_L(const Scene *scene, float u[], Ray &ray, glm::vec3 &normal, float &pdf)
{
    glm::vec3 lightPos = transform.position();
    glm::vec3 scale = transform.getScale();
    float radius = 0.5f * (scale.x + scale.y + scale.z) / 3.0f;

    normal = UniformSampleSphere(u[0],u[1]);
    pdf = UniformSpherePdf();

    glm::vec3 rayOrigin = lightPos + normal * (radius + 1e-2f);
    ray = Ray(rayOrigin, normal);

    return this->material->base_color * this->material->intensity;
}
